...
interface SimpleNewsImageUploadProps {
  newsId: number;
  newsTitle: string;
...
export const SimpleNewsImageUpload = memo(function SimpleNewsImageUpload({
  newsId,
  newsTitle,
  ...
  const [errorRecoveryState, setErrorRecoveryState] = useState<{
    ...
    retryDelay: number;
    componentSource: 'SimpleNewsImageUpload';
  ...
  // Component identification for debugging
  const componentIdRef = componentId || `simple-news-upload-${newsId}`;
  ...
  // Check if this is a new news article (newsId = 0)
  const isNewNews = newsId === 0;

  // Enhanced logging with component source identification
  const logWithTimestamp = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🔍 DEBUG: SimpleNewsImageUpload [${componentIdRef}] [${new Date().toISOString()}] - ${message}`, data || '');
    }
  }, [debugMode, componentIdRef]);

  // Show message for new news articles
  if (isNewNews) {
    return (
      <div className={`space-y-4 ${className}`}>
        <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <div className="flex items-start space-x-3">
            <Newspaper className="h-5 w-5 text-amber-600 mt-0.5 flex-shrink-0" />
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                Image Upload for New News Articles
              </p>
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                To upload an image, please create the news article first, then edit the article to add the image.
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                This ensures the image is properly associated with the news article record in the database.
              </p>
            </div>
          </div>
        </div>
  ...
  const dispatchImageUpdatedEvent = useCallback((filePath: string) => {
    const eventDetail = {
      ...
      component: 'SimpleNewsImageUpload',
    ...
    // Also dispatch a news-specific event for better targeting
    const newsEvent = new CustomEvent('newsImageUpdated', {
      detail: {
        ...eventDetail,
        newsSpecific: true,
      },
    ...
    window.dispatchEvent(newsEvent);
    document.dispatchEvent(newsEvent);
  }, [logWithTimestamp, componentIdRef, newsId]);
  ...
  const patchFormValueWithRetry = useCallback(async (
    ...
    try {
      ...
      const formValue = hasImage ? (actualImageData || `simple-news-upload-${Date.now()}`) : '';
      ...
      logWithTimestamp('Setting form value:', {
        ...
        isPlaceholder: formValue?.startsWith('simple-news-upload-'),
      });
  ...
  const currentImageQuery = useQuery(
    trpc.getSimpleNewsImage.queryOptions({
      newsId,
    })
  );
  ...
  const handleFallbackToAdvanced = useCallback(() => {
    ...
    const fallbackEvent = new CustomEvent('uploadFallbackNeeded', {
      detail: {
        fromComponent: 'SimpleNewsImageUpload',
  ...
  }, [errorRecoveryState, logWithTimestamp, componentIdRef, newsId]);
  ...
  const uploadMutation = useMutation(
    trpc.uploadSimpleNewsImage.mutationOptions({
      onMutate: () => {
      ...
      onSuccess: async (data) => {
        ...
        if (currentUploadId) {
          updateUpload(currentUploadId, {
            status: 'verified',
            progress: 100,
            filePath: `simple-news-upload-${Date.now()}`,
          });
        }
        ...
        toast.success(`✅ Image updated for ${newsTitle}`, { duration: 4000 });
        ...
        if (!patchSuccess && retryFormUpdate) {
          ...
          if (onChange) onChange(actualImageData || `simple-news-upload-${Date.now()}`);
          if (onFormValueSet) onFormValueSet(actualImageData || `simple-news-upload-${Date.now()}`);
        ...
        if (onUploadComplete) {
          onUploadComplete({
            success: true,
            filePath: actualImageData || `simple-news-upload-${Date.now()}`,
            component: 'SimpleNewsImageUpload',
            componentId: componentIdRef,
  ...
  const removeMutation = useMutation(
    trpc.removeSimpleNewsImage.mutationOptions({
      onSuccess: async () => {
        ...
        toast.success(`✅ Image removed for ${newsTitle}`, { duration: 4000 });
  ...
  }, [selectedFile, adminToken, newsId, uploadMutation, logWithTimestamp, robustFileReader, handleUploadError]);
  ...
  }, [adminToken, newsId, removeMutation]);
  ...
  }, [logWithTimestamp, newsId, isNewNews, newsTitle, hasCurrentImage, currentImageSrc, currentImageQuery.isLoading, currentImageQuery.isError, selectedFile, previewUrl, errorRecoveryState, uploadProgress, currentImageQuery.data, value]);
    ...
    <div className={`space-y-4 ${className}`}>
        ...
        {isUploading || uploadMutation.isPending ? (
          ...
          <div className="flex flex-col items-center space-y-3">
            <div className="relative">
              ...
              <Newspaper className="h-4 w-4 absolute -bottom-1 -right-1 text-blue-600" />
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload image for {newsTitle}
              </p>
      ...
      {debugMode && (
        <div className="mt-4 p-2 bg-gray-50 dark:bg-gray-700 rounded text-xs space-y-1">
          <div><strong>Debug Info (SimpleNewsImageUpload):</strong></div>
...
SimpleNewsImageUpload.displayName = 'SimpleNewsImageUpload';
