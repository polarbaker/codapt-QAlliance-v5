...
export const SimpleCaseStudyImageUpload = memo(function SimpleCaseStudyImageUpload({
  caseStudyId,
  caseStudyTitle,
  ...
  const [errorRecoveryState, setErrorRecoveryState] = useState<{
    ...
    retryDelay: number;
    componentSource: 'SimpleCaseStudyImageUpload';
  ...
  // Component identification for debugging
  const componentIdRef = componentId || `simple-case-study-upload-${caseStudyId}`;
  ...
  // Check if this is a new case study (caseStudyId = 0)
  const isNewCaseStudy = caseStudyId === 0;

  // Enhanced logging with component source identification
  const logWithTimestamp = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🔍 DEBUG: SimpleCaseStudyImageUpload [${componentIdRef}] [${new Date().toISOString()}] - ${message}`, data || '');
    }
  }, [debugMode, componentIdRef]);

  // Show message for new case studies
  if (isNewCaseStudy) {
    return (
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                Image Upload for New Case Studies
              </p>
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                To upload an image, please create the case study first, then edit the case study to add the image.
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                This ensures the image is properly associated with the case study record in the database.
              </p>
            </div>
          </div>
        </div>
  ...
  const dispatchImageUpdatedEvent = useCallback((filePath: string) => {
    const eventDetail = {
      ...
      component: 'SimpleCaseStudyImageUpload',
    ...
    // Also dispatch a case study-specific event for better targeting
    const caseStudyEvent = new CustomEvent('caseStudyImageUpdated', {
      detail: {
        ...eventDetail,
        caseStudySpecific: true,
      },
      bubbles: true
    });
    
    window.dispatchEvent(caseStudyEvent);
    document.dispatchEvent(caseStudyEvent);
  }, [logWithTimestamp, componentIdRef, caseStudyId]);
  ...
  const patchFormValueWithRetry = useCallback(async (
    ...
    try {
      ...
      const formValue = hasImage ? (actualImageData || `simple-case-study-upload-${Date.now()}`) : '';
      ...
      logWithTimestamp('Setting form value:', {
        ...
        isPlaceholder: formValue?.startsWith('simple-case-study-upload-'),
      });
  ...
  const currentImageQuery = useQuery(
    trpc.getSimpleCaseStudyImage.queryOptions({
      caseStudyId,
    })
  );
  ...
  const handleFallbackToAdvanced = useCallback(() => {
    ...
    const fallbackEvent = new CustomEvent('uploadFallbackNeeded', {
      detail: {
        fromComponent: 'SimpleCaseStudyImageUpload',
  ...
  }, [errorRecoveryState, logWithTimestamp, componentIdRef, caseStudyId]);
  ...
  const uploadMutation = useMutation(
    trpc.uploadSimpleCaseStudyImage.mutationOptions({
      onMutate: () => {
      ...
      onSuccess: async (data) => {
        ...
        if (currentUploadId) {
          updateUpload(currentUploadId, {
            status: 'verified',
            progress: 100,
            filePath: `simple-case-study-upload-${Date.now()}`,
          });
        }
        ...
        toast.success(`✅ Image updated for ${caseStudyTitle}`, { duration: 4000 });
        ...
        if (!patchSuccess && retryFormUpdate) {
          ...
          if (onChange) onChange(actualImageData || `simple-case-study-upload-${Date.now()}`);
          if (onFormValueSet) onFormValueSet(actualImageData || `simple-case-study-upload-${Date.now()}`);
        ...
        if (onUploadComplete) {
          onUploadComplete({
            success: true,
            filePath: actualImageData || `simple-case-study-upload-${Date.now()}`,
            component: 'SimpleCaseStudyImageUpload',
            componentId: componentIdRef,
  ...
  const removeMutation = useMutation(
    trpc.removeSimpleCaseStudyImage.mutationOptions({
      onSuccess: async () => {
        ...
        toast.success(`✅ Image removed for ${caseStudyTitle}`, { duration: 4000 });
  ...
  }, [selectedFile, adminToken, caseStudyId, uploadMutation, logWithTimestamp, robustFileReader, handleUploadError]);
  ...
  }, [adminToken, caseStudyId, removeMutation]);
  ...
  }, [logWithTimestamp, caseStudyId, isNewCaseStudy, caseStudyTitle, hasCurrentImage, currentImageSrc, currentImageQuery.isLoading, currentImageQuery.isError, selectedFile, previewUrl, errorRecoveryState, uploadProgress, currentImageQuery.data, value]);

  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Image for {caseStudyTitle}
        </h3>
        {hasCurrentImage && (
          <button
        ...
        {isUploading || uploadMutation.isPending ? (
          ...
          <div className="flex flex-col items-center space-y-3">
            <div className="relative">
              ...
              <BookOpen className="h-4 w-4 absolute -bottom-1 -right-1 text-blue-600" />
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload image for {caseStudyTitle}
              </p>
      ...
      {debugMode && (
        <div className="mt-4 p-2 bg-gray-50 dark:bg-gray-700 rounded text-xs space-y-1">
          <div><strong>Debug Info (SimpleCaseStudyImageUpload):</strong></div>
...
SimpleCaseStudyImageUpload.displayName = 'SimpleCaseStudyImageUpload';
