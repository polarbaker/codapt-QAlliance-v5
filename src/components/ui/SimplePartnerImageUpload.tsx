...
interface SimplePartnerImageUploadProps {
  partnerId: number;
  partnerName: string;
...
export const SimplePartnerImageUpload = memo(function SimplePartnerImageUpload({
  partnerId,
  partnerName,
  ...
  const [errorRecoveryState, setErrorRecoveryState] = useState<{
    ...
    retryDelay: number;
    componentSource: 'SimplePartnerImageUpload';
  ...
  // Component identification for debugging
  const componentIdRef = componentId || `simple-partner-upload-${partnerId}`;
  ...
  // Check if this is a new partner (partnerId = 0)
  const isNewPartner = partnerId === 0;

  // Enhanced logging with component source identification
  const logWithTimestamp = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🔍 DEBUG: SimplePartnerImageUpload [${componentIdRef}] [${new Date().toISOString()}] - ${message}`, data || '');
    }
  }, [debugMode, componentIdRef]);

  // Show message for new partners
  if (isNewPartner) {
    return (
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                Logo Upload for New Partners
              </p>
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                To upload a logo, please create the partner first, then edit the partner to add the logo.
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                This ensures the logo is properly associated with the partner record in the database.
              </p>
            </div>
          </div>
        </div>
  ...
  const dispatchImageUpdatedEvent = useCallback((filePath: string) => {
    const eventDetail = {
      ...
      component: 'SimplePartnerImageUpload',
    ...
    // Also dispatch a partner-specific event for better targeting
    const partnerEvent = new CustomEvent('partnerImageUpdated', {
      detail: {
        ...eventDetail,
    ...
    document.dispatchEvent(partnerEvent);
  }, [logWithTimestamp, componentIdRef, partnerId]);
  ...
  const patchFormValueWithRetry = useCallback(async (
    ...
    try {
      ...
      const formValue = hasImage ? (actualImageData || `simple-partner-upload-${Date.now()}`) : '';
      ...
      logWithTimestamp('Setting form value:', {
        ...
        isPlaceholder: formValue?.startsWith('simple-partner-upload-'),
      });
  ...
  const currentImageQuery = useQuery(
    trpc.getSimplePartnerImage.queryOptions({
      partnerId,
    })
  );
  ...
  const handleFallbackToAdvanced = useCallback(() => {
    ...
    const fallbackEvent = new CustomEvent('uploadFallbackNeeded', {
      detail: {
        fromComponent: 'SimplePartnerImageUpload',
  ...
  }, [errorRecoveryState, logWithTimestamp, componentIdRef, partnerId]);
  ...
  const uploadMutation = useMutation(
    trpc.uploadSimplePartnerImage.mutationOptions({
      onMutate: () => {
      ...
      onSuccess: async (data) => {
        ...
        if (currentUploadId) {
          updateUpload(currentUploadId, {
            status: 'verified',
            progress: 100,
            filePath: `simple-partner-upload-${Date.now()}`,
          });
        }
        ...
        toast.success(`✅ Logo updated for ${partnerName}`, { duration: 4000 });
        ...
        if (!patchSuccess && retryFormUpdate) {
          ...
          if (onChange) onChange(actualImageData || `simple-partner-upload-${Date.now()}`);
          if (onFormValueSet) onFormValueSet(actualImageData || `simple-partner-upload-${Date.now()}`);
        ...
        if (onUploadComplete) {
          onUploadComplete({
            success: true,
            filePath: actualImageData || `simple-partner-upload-${Date.now()}`,
            component: 'SimplePartnerImageUpload',
            componentId: componentIdRef,
  ...
  const removeMutation = useMutation(
    trpc.removeSimplePartnerImage.mutationOptions({
      onSuccess: async () => {
        ...
        toast.success(`✅ Logo removed for ${partnerName}`, { duration: 4000 });
  ...
  }, [selectedFile, adminToken, partnerId, uploadMutation, logWithTimestamp, robustFileReader, handleUploadError]);
  ...
  }, [adminToken, partnerId, removeMutation]);
  ...
  }, [logWithTimestamp, partnerId, isNewPartner, partnerName, hasCurrentImage, currentImageSrc, currentImageQuery.isLoading, currentImageQuery.isError, selectedFile, previewUrl, errorRecoveryState, uploadProgress, currentImageQuery.data, value]);

  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Logo for {partnerName}
        </h3>
        {hasCurrentImage && (
          <button
        ...
        {isUploading || uploadMutation.isPending ? (
          ...
          <div className="flex flex-col items-center space-y-3">
            <div className="relative">
              ...
              <Building className="h-4 w-4 absolute -bottom-1 -right-1 text-blue-600" />
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload logo for {partnerName}
              </p>
      ...
      {currentImageQuery.isLoading && (
        <div className="flex items-center justify-center py-4">
          ...
          <span className="ml-2 text-sm text-gray-500">Loading current logo...</span>
        </div>
      )}
        ...
        <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm text-amber-700 dark:text-amber-300">
              <p>Could not load current logo</p>
              <button
      ...
      {debugMode && (
        <div className="mt-4 p-2 bg-gray-50 dark:bg-gray-700 rounded text-xs space-y-1">
          <div><strong>Debug Info (SimplePartnerImageUpload):</strong></div>
...
SimplePartnerImageUpload.displayName = 'SimplePartnerImageUpload';
