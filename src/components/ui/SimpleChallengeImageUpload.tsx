...
interface SimpleChallengeImageUploadProps {
  challengeId: number;
  challengeTitle: string;
  // Standardized interface to match other Simple upload components
  value?: string;
...
export const SimpleChallengeImageUpload = memo(function SimpleChallengeImageUpload({
  challengeId,
  challengeTitle,
  ...
  componentId,
}: SimpleChallengeImageUploadProps) {
  ...
  const [errorRecoveryState, setErrorRecoveryState] = useState<{
    ...
    retryDelay: number;
    componentSource: 'SimpleChallengeImageUpload';
  ...
  // Component identification for debugging
  const componentIdRef = componentId || `simple-challenge-upload-${challengeId}`;
  ...
  // Check if this is a new challenge (challengeId = 0)
  const isNewChallenge = challengeId === 0;

  // Enhanced logging with component source identification
  const logWithTimestamp = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🔍 DEBUG: SimpleChallengeImageUpload [${componentIdRef}] [${new Date().toISOString()}] - ${message}`, data || '');
    }
  }, [debugMode, componentIdRef]);

  // Show message for new challenges
  if (isNewChallenge) {
    return (
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-800 dark:text-blue-200">
                Image Upload Available After Creation
              </p>
              <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                You can create the challenge now and add an image later by editing the challenge.
              </p>
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-2">
                This workflow ensures reliable image uploads and proper database associations.
              </p>
            </div>
          </div>
        </div>
        
        {/* Provide a placeholder that doesn't interfere with form submission */}
        <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center bg-gray-50 dark:bg-gray-800">
          <div className="flex flex-col items-center space-y-3">
            <div className="relative opacity-50">
              ...
              <Trophy className="h-4 w-4 absolute -bottom-1 -right-1 text-blue-600" />
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Image upload will be available after challenge creation
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Create challenge → Edit → Upload image
              </p>
            </div>
          </div>
        </div>
  ...
  // Helper function to dispatch custom imageUpdated event
  const dispatchImageUpdatedEvent = useCallback((filePath: string) => {
    const eventDetail = {
      filePath: filePath,
      timestamp: Date.now(),
      uploadedAt: new Date().toISOString(),
      component: 'SimpleChallengeImageUpload',
    ...
    logWithTimestamp('Dispatching imageUpdated event:', {
      ...
      isBase64: filePath?.startsWith('data:'),
    });
    ...
    // Also dispatch a challenge-specific event for better targeting
    const challengeEvent = new CustomEvent('challengeImageUpdated', {
      detail: {
        ...eventDetail,
        challengeSpecific: true,
      },
      bubbles: true
    });
  ...
  }, [logWithTimestamp, componentIdRef, challengeId]);

  // Enhanced form value patching with retry logic
  const patchFormValueWithRetry = useCallback(async (
    hasImage: boolean,
    retryCount: number = 0,
    maxRetries: number = 3,
    actualImageData?: string // Add parameter for actual image data
  ): Promise<boolean> => {
    try {
      logWithTimestamp('patchFormValueWithRetry called:', {
        ...
        maxRetries,
        currentValue: value,
        actualImageData: actualImageData ? `Present (${actualImageData.length} chars)` : 'None',
        componentSource: 'SimpleChallengeImageUpload',
      });

      // Generate appropriate form value - ALWAYS use actual image data if available
      const formValue = hasImage ? (actualImageData || `simple-challenge-upload-${Date.now()}`) : '';
      
      logWithTimestamp('Setting form value:', {
        hasImage,
        formValue: formValue ? `Present (${formValue.length} chars)` : 'None',
        isBase64: formValue?.startsWith('data:'),
        isPlaceholder: formValue?.startsWith('simple-challenge-upload-'),
      });
      ...
      logWithTimestamp('Form value update verification:', {
        ...
        expectedValue: formValue ? `Present (${formValue.length} chars)` : 'None',
        actualValue: value ? `Present (${value.length} chars)` : 'None',
      ...
      if (updateSuccessful || retryCount >= maxRetries) {
        if (updateSuccessful) {
          logWithTimestamp('Form value successfully updated - dispatching event with actual data');
          
          // Dispatch custom event for crosscutting updates with ACTUAL image data
          if (hasImage && formValue) {
            dispatchImageUpdatedEvent(formValue); // Use actual image data, not placeholder
          }
      ...
      return patchFormValueWithRetry(hasImage, retryCount + 1, maxRetries, actualImageData);
  ...
  const currentImageQuery = useQuery(
    trpc.getSimpleChallengeImage.queryOptions({
      challengeId,
    })
  );
  ...
  const handleFallbackToAdvanced = useCallback(() => {
    ...
    const fallbackEvent = new CustomEvent('uploadFallbackNeeded', {
      detail: {
        fromComponent: 'SimpleChallengeImageUpload',
  ...
  }, [errorRecoveryState, logWithTimestamp, componentIdRef, challengeId]);
  ...
  const uploadMutation = useMutation(
    trpc.uploadSimpleChallengeImage.mutationOptions({
      onMutate: () => {
      ...
      onSuccess: async (data) => {
        ...
        if (currentUploadId) {
          updateUpload(currentUploadId, {
            status: 'verified',
            progress: 100,
            filePath: `simple-challenge-upload-${Date.now()}`,
          });
        }
        ...
        // Refetch current image to get the actual base64 data
        const updatedImageData = await currentImageQuery.refetch();
        
        toast.success(`✅ Image updated for ${challengeTitle}`, { duration: 4000 });
        
        // Set form field to actual image data instead of placeholder
        let actualImageData = '';
        if (updatedImageData.data) {
          if (updatedImageData.data.isBase64 && updatedImageData.data.imageData) {
            actualImageData = updatedImageData.data.imageData;
          } else if (!updatedImageData.data.isBase64 && updatedImageData.data.imageUrl) {
            actualImageData = updatedImageData.data.imageUrl;
          }
        }
        
        logWithTimestamp('Setting form field to actual image data:', {
          hasActualData: !!actualImageData,
          dataType: actualImageData.startsWith('data:') ? 'base64' : 'url',
          dataLength: actualImageData.length,
        });
        
        // Enhanced form patching with actual image data
        const patchSuccess = await patchFormValueWithRetry(true, 0, 3, actualImageData);
        
        if (!patchSuccess && retryFormUpdate) {
          logWithTimestamp('Form patching failed, attempting direct fallback with actual data');
          // Fallback: try direct callbacks with actual image data
          if (onChange) onChange(actualImageData || `simple-challenge-upload-${Date.now()}`);
          if (onFormValueSet) onFormValueSet(actualImageData || `simple-challenge-upload-${Date.now()}`);
        ...
        if (onUploadComplete) {
          onUploadComplete({
            success: true,
            filePath: actualImageData || `simple-challenge-upload-${Date.now()}`,
            component: 'SimpleChallengeImageUpload',
            componentId: componentIdRef,
  ...
  const removeMutation = useMutation(
    trpc.removeSimpleChallengeImage.mutationOptions({
      onSuccess: async () => {
        ...
        toast.success(`✅ Image removed for ${challengeTitle}`, { duration: 4000 });
  ...
  }, [selectedFile, adminToken, challengeId, uploadMutation, logWithTimestamp, robustFileReader, handleUploadError]);
  ...
  }, [adminToken, challengeId, removeMutation]);
  ...
  }, [logWithTimestamp, challengeId, isNewChallenge, challengeTitle, hasCurrentImage, currentImageSrc, currentImageQuery.isLoading, currentImageQuery.isError, selectedFile, previewUrl, errorRecoveryState, uploadProgress, currentImageQuery.data, value]);

  return (
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Image for {challengeTitle}
        </h3>
        {hasCurrentImage && (
          <button
      ...
      >
        ...
        {isUploading || uploadMutation.isPending ? (
          ...
          <div className="flex flex-col items-center space-y-3">
            ...
            <div className="space-y-1">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload image for {challengeTitle}
              </p>
      ...
      {currentImageQuery.isLoading && (
        <div className="flex items-center justify-center py-4">
          ...
          <span className="ml-2 text-sm text-gray-500">Loading current image...</span>
        </div>
      )}
        ...
        <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm text-amber-700 dark:text-amber-300">
              <p>Could not load current image</p>
              <button
      ...
      {debugMode && (
        <div className="mt-4 p-2 bg-gray-50 dark:bg-gray-700 rounded text-xs space-y-1">
          <div><strong>Debug Info (SimpleChallengeImageUpload):</strong></div>
...
SimpleChallengeImageUpload.displayName = 'SimpleChallengeImageUpload';
